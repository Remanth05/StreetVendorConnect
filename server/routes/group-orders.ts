...
export const handleCreateGroupOrder: RequestHandler = (req, res) => {
  const { title, description, category, targetQuantity, deadline, userEmail } = req.body;
  
  if (!title || !description || !category || !targetQuantity || !deadline || !userEmail) {
    return res.status(400).json({ 
  ...
  res.json({
    success: true,
    message: 'Group order created successfully! We will help you find participants.',
