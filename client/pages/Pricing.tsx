...
const clientPlans: PricingPlan[] = [
  ...
  },
  {
    id: 'client-pro',
    name: 'Professional',
    description: 'For growing businesses that need better supplier relationships',
    price: 999,
    period: 'month',
    popular: true,
    features: [
...
export default function Pricing() {
  const { user } = useAuth();
  const [selectedTab, setSelectedTab] = useState(user?.type === 'supplier' ? 'suppliers' : 'clients');
  ...
  const handleSelectPlan = (plan: PricingPlan) => {
    ...
    if (plan.price === 0) {
      alert(`You're already on the ${plan.name} plan! Start exploring the platform.`);
    } else {
      alert(`Plan selection: ${plan.name} - ₹${getDiscountedPrice(plan.price)}/${billingPeriod === 'yearly' ? 'year' : 'month'}. Our team will contact you to complete the setup.`);
    }
  };
              ...
              >
                ...
                <CardHeader className="text-center pb-4">
                  ...
                  <div className="mt-4">
                    <div className="flex items-baseline justify-center">
                      <span className="text-4xl font-bold text-gray-900">
                        ₹{plan.price === 0 ? '0' : getDiscountedPrice(plan.price).toLocaleString()}
                      </span>
                      {plan.price > 0 && (
                  ...
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    <div className="text-sm font-medium text-gray-900">{plan.savings}</div>
                    <div className="text-xs text-gray-600 mt-1">vs. traditional sourcing</div>
                  </div>
                </CardHeader>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    {getPlans().map((plan) => (
                      <th key={plan.id} className="text-center py-4 px-4 min-w-32">
                        <div className="font-semibold">{plan.name}</div>
                        <div className="text-sm text-gray-500">
                          ₹{plan.price === 0 ? '0' : getDiscountedPrice(plan.price).toLocaleString()}
                          {plan.price > 0 && `/${billingPeriod === 'yearly' ? 'yr' : 'mo'}`}
                        </div>
                      </th>
                    ))}
                  </tr>
                    ...
                    <tr key={feature} className="border-b">
                      ...
                      {getPlans().map((plan, index) => (
                        <td key={plan.id} className="py-3 px-4 text-center">
                          {index === 0 ? (
                            feature === 'Supplier Access' || feature === 'Product Listings' || feature === 'Order Management' ? (
                              <Check className="h-5 w-5 text-green-500 mx-auto" />
                            ) : (
                              <X className="h-5 w-5 text-gray-300 mx-auto" />
                            )
                          ) : index === 1 ? (
                            feature === 'API Access' || feature === 'Dedicated Account Manager' ? (
                              <X className="h-5 w-5 text-gray-300 mx-auto" />
                            ) : (
                              <Check className="h-5 w-5 text-green-500 mx-auto" />
                            )
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <h4 className="font-semibold mb-2">Can I get a custom plan?</h4>
                  <p className="text-sm text-gray-600">
                    Absolutely! Contact us for enterprise-level custom solutions tailored to your needs.
                  </p>
                </div>
                <div>
                  <h4 className="font-semibold mb-2">How does billing work?</h4>
                  <p className="text-sm text-gray-600">
                    Billing is automatic and processed monthly or yearly based on your chosen plan.
                  </p>
                </div>
              </div>
            </div>
        ...
        <Card className="bg-gradient-to-r from-brand-green to-brand-green-dark text-white border-0">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-green-100 mb-6">
              Join thousands of businesses already saving money and time with StreetVendor Connect.
            </p>
              ...
              <Button 
                ...
                onClick={() => {
                  if (!user) {
                    ...
                    alert('Great! Your free trial is already active. Start exploring the platform!');
                  }
                }}
              >
                Start Free Trial
