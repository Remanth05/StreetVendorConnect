...
const mockGroupOrders: GroupOrder[] = [
  {
    ...
    description: 'High-quality fresh vegetables for restaurants and food vendors. Sourced directly from farms.',
    category: 'Vegetables',
  ...
  },
  {
    ...
    description: 'Authentic Indian spice mixes for commercial kitchens. Premium quality guaranteed.',
    category: 'Spices',
  ...
  },
  {
    ...
    description: 'Sustainable packaging solutions for food vendors. Biodegradable and eco-friendly options.',
    category: 'Packaging',
  ...
  },
  {
    ...
    description: 'High-quality cooking oils for commercial use. Various types available.',
    category: 'Oil',
...
export default function GroupOrders() {
  ...
  const categories = ["All", "Vegetables", "Spices", "Packaging", "Oil", "Dairy", "Grains"];
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'open': return 'Open for Joining';
      case 'filling': return 'Almost Full';
  ...
  const joinGroupOrder = (order: GroupOrder, quantity: number) => {
    ...
    alert(`Successfully joined "${order.title}" with quantity: ${quantity} ${order.unit}. You'll receive updates on your dashboard.`);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-4">Group Orders</h1>
              <p className="text-gray-600">
                Join group purchases to save money and get better deals on bulk orders
              </p>
            </div>
            <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Start a new group order to get better prices for bulk purchases
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button 
                      onClick={() => {
                        alert('Group order request submitted! We\'ll help you set it up and find participants.');
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {mockGroupOrders.filter(o => o.status === 'open' || o.status === 'filling').length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    {mockGroupOrders.reduce((sum, order) => sum + order.currentParticipants, 0)}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {Math.round(mockGroupOrders.reduce((sum, order) => sum + order.savings, 0) / mockGroupOrders.length)}%
                  </p>
                </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">
                        {order.currentParticipants}/{order.maxParticipants} participants
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-gray-600">
                        ...
                        <span>Min: {order.minOrder} {order.unit}</span>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 text-gray-600">
                        <Calendar className="h-4 w-4" />
                        <span>Ends {new Date(order.deadline).toLocaleDateString()}</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-600">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const quantity = prompt(`How many ${order.unit} would you like to order? (Min: ${order.minOrder})`);
                          if (quantity && parseInt(quantity) >= order.minOrder) {
                            joinGroupOrder(order, parseInt(quantity));
                          } else if (quantity) {
                            alert(`Minimum order is ${order.minOrder} ${order.unit}`);
                          }
                        }}
                      >
                        {order.status === 'open' ? 'Join Order' : order.status === 'filling' ? 'Almost Full' : 'Closed'}
                      </Button>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1 text-sm">
                        ...
                        <div>Price: ₹{selectedOrder.pricePerUnit} per {selectedOrder.unit}</div>
                        <div>Min Order: {selectedOrder.minOrder} {selectedOrder.unit}</div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1 text-sm">
                        <div>Deadline: {new Date(selectedOrder.deadline).toLocaleDateString()}</div>
                        <div>Delivery: {new Date(selectedOrder.deliveryDate).toLocaleDateString()}</div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {selectedOrder.products.map((product) => (
                        <div key={product} className="p-2 bg-gray-50 rounded text-sm text-center">
                          {product}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-2 pt-4">
                    <Button
                      ...
                      onClick={() => {
                        const quantity = prompt(`How many ${selectedOrder.unit} would you like to order? (Min: ${selectedOrder.minOrder})`);
                        if (quantity && parseInt(quantity) >= selectedOrder.minOrder) {
                          joinGroupOrder(selectedOrder, parseInt(quantity));
                          setSelectedOrder(null);
                        } else if (quantity) {
                          alert(`Minimum order is ${selectedOrder.minOrder} ${selectedOrder.unit}`);
                        }
                      }}
                    >
                      {selectedOrder.status === 'open' ? 'Join This Order' : 'Order Closed'}
                    </Button>
                    <Button
                      variant="outline"
                      onClick={() => {
        ...
        <Card className="mt-8 bg-gradient-to-r from-brand-green to-brand-green-dark text-white border-0">
          <CardContent className="p-8 text-center">
            <h3 className="text-2xl font-bold mb-4">Want to organize your own group order?</h3>
            <p className="text-green-100 mb-6">
              Create a group order and let others join to get better wholesale prices for everyone.
            </p>
            <Button 
